...
import com.atum.net.codec.builder.AccessMode;
import com.atum.net.codec.builder.DataConstants;
import com.atum.net.codec.builder.DataOrder;
...
public final class GamePacketBuilder {
	...
	public GamePacketBuilder() {
		opcode = -1;
		type = PacketHeader.RAW;
	}

	/**
	 * Creates the {@link GamePacketBuilder} for a {@link PacketHeader#FIXED} packet with the specified opcode.
	 *
	 ...
	 */
	public GamePacketBuilder(int opcode, PacketHeader type) {
		this.opcode = opcode;
		this.type = type;
	}
	...
	public void putRawBuilder(GamePacketBuilder builder) {
		checkByteAccess();
		if (builder.type != PacketHeader.RAW) {
	...
	public void putRawBuilderReverse(GamePacketBuilder builder) {
		...
		Preconditions.checkArgument(builder.type == PacketHeader.RAW, "Builder must be raw.");
	...
	public void putString(String str) {
		...
		buffer.writeByte(DataConstants.STRING_TERMINATOR);
	}
	...
	public GamePacket toGamePacket() {
		Preconditions.checkState(type != PacketHeader.RAW, "Raw packets cannot be converted to a game packet.");
